...
class FactionMixin:
    ...
    def bar_player(self, planet_name, duration_hours=None):
            """Bar player from a planet for a configurable number of hours."""
            if not self.player:
                return

            if duration_hours is None:
                duration_hours = float(self.config.get("planet_loss_travel_ban_hours", 6))
            hours = max(0.25, float(duration_hours))
            expiry = time.time() + (hours * 3600.0)
            self.player.barred_planets[str(planet_name)] = expiry

            planet = self.get_planet_by_id(planet_name) or self.get_planet_by_name(planet_name)
            if planet is not None:
                self.player.barred_planets[str(getattr(planet, "planet_id", ""))] = expiry
                self.player.barred_planets[str(getattr(planet, "name", ""))] = expiry
