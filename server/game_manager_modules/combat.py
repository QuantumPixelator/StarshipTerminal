...
class CombatMixin:
    ...
    def start_combat_session(self, target_data):
        ...
        if bool(getattr(self.player, "is_docked", False)):
            return False, "Combat requires orbit engagement. Undock first.", None
        ...
        if target_type == "NPC":
            ...
            if target is None:
                ...
                compact_stats = (
                    target_data.get("stats")
                    if isinstance(target_data.get("stats"), dict)
                    else {}
                )
                ship_raw = (
                ...
                if not ship_raw and compact_stats:
                    ship_raw = {
                        "current_shields": int(compact_stats.get("shields", 0) or 0),
                        "current_defenders": int(compact_stats.get("defenders", 0) or 0),
                        "integrity": int(compact_stats.get("integrity", 100) or 100),
                    }
                if compact_stats:
                    if "credits" not in npc_raw:
                        npc_raw["credits"] = int(compact_stats.get("credits", 0) or 0)
                    if "name" not in npc_raw and target_data.get("name"):
                        npc_raw["name"] = str(target_data.get("name"))
                    if "personality" not in npc_raw and target_data.get("personality"):
                        npc_raw["personality"] = str(target_data.get("personality"))
            ...
            # Prefer server-side cached orbit target data (avoids sending full
            # save blobs to the client).  Fall back to raw_data if the client
            # is running an older version that still echoes it.
            _cached = (getattr(self, "_cached_orbit_targets", {}) or {}).get(target_name)
            if _cached and isinstance(_cached.get("raw_data"), dict):
                raw = _cached["raw_data"].get("player", {})
            elif isinstance(target_data.get("raw_data"), dict):
                raw = target_data["raw_data"].get("player", {})
            else:
                raw = {}
            s_raw = raw.get("spaceship", {})
            ...
            target_frontier = int(raw.get("frontier_standing", 0) or 0)
            target_authority = int(
                raw.get("authority_standing", raw.get("sector_reputation", 0)) or 0
            )
            contraband_names = set(self.get_smuggling_item_names())
            target_contraband_units = int(
                sum(
                    max(0, int(qty or 0))
                    for item, qty in session["target_inventory"].items()
                    if str(item) in contraband_names
                )
            )
            session["target_frontier_standing"] = int(target_frontier)
            session["target_authority_standing"] = int(target_authority)
            session["target_has_contraband"] = bool(target_contraband_units > 0)
            session["target_start"] = {
            ...
            # Determine save path from cache or fallback filename
            target_file = (
                str(_cached.get("save_path", "")).strip() if _cached else
                str(target_data.get("save_path", "")).strip()
            )
            if target_file:
                session["target_file"] = target_file
            else:
                session["target_file"] = self._find_commander_save_path_by_name(target_name)
        else:
    ...
    def _finish_combat_session(self, session, player_won):
        ...
        if player_won:
            ...
            if target_type == "PLAYER":
                target_frontier = int(session.get("target_frontier_standing", 0))
                target_authority = int(session.get("target_authority_standing", 0))
                target_smuggler = bool(session.get("target_has_contraband", False))

                if target_smuggler or target_frontier >= 60:
                    auth_gain = 4 if target_smuggler else 2
                    self._adjust_authority_standing(auth_gain)

                if target_authority >= 60:
                    self._adjust_frontier_standing(2)
            ...
            if target_type == "PLANET":
                ...
                conquered_planet_id = str(getattr(self.current_planet, "planet_id", ""))
                self.player.owned_planets[conquered_planet_id] = time.time()
                self.current_planet.last_defense_regen_time = time.time()
                self._clear_planet_attack_state(target_name)

                # Ensure prior owner's ownership ledger is cleared immediately.
                if old_owner and old_owner != self.player.name:
                    for owner_ref in list(self._find_commander_save_paths_by_name(old_owner) or []):
                        if not str(owner_ref).startswith("db://") or getattr(self, "store", None) is None:
                            continue
                        try:
                            _, _, remainder = str(owner_ref).partition("db://")
                            account_name, _, character_name = remainder.partition("/")
                            old_payload = self.store.get_character_payload(account_name, character_name)
                            player_payload = old_payload.get("player") if isinstance(old_payload, dict) else None
                            owned_map = (
                                player_payload.get("owned_planets")
                                if isinstance(player_payload, dict)
                                else None
                            )
                            if isinstance(owned_map, dict) and conquered_planet_id in owned_map:
                                owned_map.pop(conquered_planet_id, None)
                                self.store.upsert_character_payload(
                                    account_name,
                                    character_name,
                                    old_payload,
                                    display_name=str(
                                        (old_payload.get("player") or {}).get("name")
                                        or character_name
                                    ),
                                )
                        except Exception:
                ...
                treasury_balance = int(getattr(self.current_planet, "credit_balance", 0) or 0)
                if treasury_balance > 0:
                    result_text += f" TREASURY CONTROL TRANSFERRED: {treasury_balance:,} CR NOW UNDER YOUR COMMAND."
            else:
                result_text = f"TARGET DISABLED: {target_name}."

            if target_type == "PLAYER":
                path = session.get("target_file")
                if path and path.startswith("db://") and getattr(self, "store", None) is not None:
                    try:
                        ...
                        if not isinstance(data, dict) or not isinstance(data.get("player"), dict):
                            data = None
                        if data is None:
                            raise ValueError("Target payload missing")
                        if not isinstance(data["player"].get("spaceship"), dict):
                            data["player"]["spaceship"] = {}
            ...
            if target_type == "PLANET":
                owner_now = str(getattr(self.current_planet, "owner", "") or "").strip().lower()
                if owner_now in {"", "unclaimed", "none", "null"}:
                    ban_hours = max(
                        0.25,
                        float(self.config.get("planet_loss_travel_ban_hours", 6)),
                    )
                    self.bar_player(
                        str(getattr(self.current_planet, "planet_id", "")),
                        duration_hours=ban_hours,
                    )
                    result_text += (
                        f" NAVIGATION ACCESS TO {target_name.upper()} SUSPENDED "
                        f"FOR {ban_hours:.1f} HOURS."
                    )
            elif target_type == "PLAYER":
                target_frontier = int(session.get("target_frontier_standing", 0))
                target_authority = int(session.get("target_authority_standing", 0))
                target_smuggler = bool(session.get("target_has_contraband", False))
                if target_smuggler or target_frontier >= 60:
                    self._adjust_authority_standing(-2)
                if target_authority >= 60:
                    self._adjust_frontier_standing(-2)
    ...
    def resolve_combat_round(self, session, player_committed):
        ...
        per_round_fuel = max(1.0, float(self.config.get("combat_fuel_use_per_round", 6)))
        fuel_ok, fuel_msg = self.consume_fuel(
            getattr(self.player.spaceship, "model", None),
            per_round_fuel,
        )
        if not fuel_ok:
            self._finish_combat_session(session, player_won=False)
            session.setdefault("log", []).append("FUEL EXHAUSTED DURING ENGAGEMENT.")
            return True, "Fuel exhausted. Combat capability collapsed.", session
        ...
        if fuel_msg and "LOW FUEL" in str(fuel_msg).upper():
            round_lines.append("LOW FUEL PENALTY: THRUST AND TARGETING DRIFT INCREASED.")
    ...
    def resolve_combat(self, target_data):
        ...
        if target_data["type"] == "NPC":
            ...
            target_name = target_data["name"]
            # Use server-side cached orbit target data when available
            _cached = (getattr(self, "_cached_orbit_targets", {}) or {}).get(target_name)
            if _cached and isinstance(_cached.get("raw_data"), dict):
                raw = _cached["raw_data"].get("player", {})
            elif isinstance(target_data.get("raw_data"), dict):
                ...
                raw = {}
            s_raw = raw.get("spaceship", {})
            t_credits = raw.get("credits", 0)
            t_inv = raw.get("inventory", {})
            t_defenders = s_raw.get("current_defenders", 0)
            # Mock ship for calc
            from classes import Spaceship

            t_ship = Spaceship(s_raw.get("model", "Unknown"), 0, 0, 0, 0, 0, 0, 0)
            t_ship.current_shields = int(s_raw.get("current_shields", 0))
            t_ship.current_defenders = int(s_raw.get("current_defenders", 0))
            t_ship.integrity = int(s_raw.get("integrity", 100))
        ...
        if p_roll >= t_roll:
            ...
            if is_planet_target:
                ...
                if planet.shields == 0 and planet.defenders == 0:
                    ...
                    self.player.owned_planets[str(getattr(planet, "planet_id", ""))] = time.time()
            ...
            if target_data["type"] == "PLAYER":
                ...
                path = self._find_commander_save_path_by_name(target_name)
                try:
                    if not path.startswith("db://") or getattr(self, "store", None) is None:
