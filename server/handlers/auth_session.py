...
def _h_logout_commander(server, session, gm, params):
    ...
    return {
        ...
        "logged_out_commander": character_name,
    }
...
def _h_new_game(server, session, gm, params):
    ...
    try:
        ...
        # Server-side: prevent duplicate commander names globally.
        if requested_safe in linked_names or server._is_commander_name_taken(player_name):
            log_validation_error(
...
def _h_load_game(server, session, gm, params):
    if gm is None:
        return {"success": False, "message": "Session game manager unavailable."}
    account_safe = getattr(session, "_account_safe", None)
...
def _h_save_game(server, session, gm, params):
    if gm is None:
        return {"success": False, "message": "Session game manager unavailable."}
    success = gm.save_game(force=True)
...
def _h_list_saves(server, session, gm, params):
    ...
    if account_safe:
        ...
        if gm is None:
            return {"success": False, "saves": [], "message": "Session game manager unavailable."}
        saves = gm.list_saves()
