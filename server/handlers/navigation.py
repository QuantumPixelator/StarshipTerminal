...
def _h_warp_to_planet(server, session, gm, params):
    target_planet = gm.resolve_planet_from_params(params, default_current=False)
    if target_planet is None:
        return {"success": False, "message": "Invalid target planet."}
    ...
    gm.record_analytics_event(
        ...
        metadata={
            "planet_id": str(getattr(target_planet, "planet_id", "") or ""),
...
def _h_travel_to_planet(server, session, gm, params):
    target_idx = _as_int(params.get("target_planet_index"), default=None)
    if target_idx is None or int(target_idx) < 0:
        return {"success": False, "message": "Invalid target_planet_index."}
...
def _h_dock_current_planet(server, session, gm, params):
    success, msg = gm.dock_current_planet()
    gm.record_analytics_event(
        category="navigation",
        event_name="dock_current_planet",
        success=bool(success),
        metadata={"planet": str(getattr(getattr(gm, "current_planet", None), "name", "") or "")},
    )
    return {"success": bool(success), "message": str(msg)}


def _h_undock_current_planet(server, session, gm, params):
    success, msg = gm.undock_current_planet()
    gm.record_analytics_event(
        category="navigation",
        event_name="undock_current_planet",
        success=bool(success),
        metadata={"planet": str(getattr(getattr(gm, "current_planet", None), "name", "") or "")},
    )
    return {"success": bool(success), "message": str(msg)}
...
def _h_roll_travel_event_payload(server, session, gm, params):
    target_planet = gm.resolve_planet_from_params(params, default_current=False)
    if target_planet is None:
        return {"success": False, "payload": None, "message": "Invalid target planet."}
    try:
        dist = float(params.get("dist", 0.0))
    except Exception:
        return {"success": False, "payload": None, "message": "Invalid dist."}
    payload = (
...
def register():
    return {
        ...
        "dock_current_planet": _h_dock_current_planet,
        "undock_current_planet": _h_undock_current_planet,
