...
def _h_buy_fuel(server, session, gm, params):
    amount = max(1, _as_int(params.get("amount", 10), default=10))
...
def _h_buy_ship(server, session, gm, params):
    ship_name = str(params.get("ship") or "").strip()
    if not ship_name:
        return {
            "success": False,
            "message": "Invalid ship model.",
        }
    catalog_ship = next(
        (
            s
...
def _h_transfer_fighters(server, session, gm, params):
    action_type = str(params.get("action") or "").strip().upper()
    if action_type == "TO_HOLD":
        action_type = "TO_PLANET"
    if action_type not in {"TO_PLANET", "TO_SHIP"}:
...
def _h_install_ship_upgrade(server, session, gm, params):
    ...
    try:
        item_name = str(params.get("item_name") or "").strip()
        quantity = max(1, _as_int(params.get("quantity", 1), default=1))
        if not item_name:
            return {
                "success": False,
