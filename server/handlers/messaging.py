...
def _h_has_unseen_galactic_news(server, session, gm, params):
    ...
    if lookback_days is not None:
        lookback_days = _as_int(lookback_days, default=None)
        if lookback_days is None or lookback_days < 0:
            return {"success": False, "message": "Invalid lookback_days."}
...
def _h_get_unseen_galactic_news(server, session, gm, params):
    ...
    if lookback_days is not None:
        lookback_days = _as_int(lookback_days, default=None)
        if lookback_days is None or lookback_days < 0:
            return {"success": False, "entries": [], "message": "Invalid lookback_days."}
...
def _h_gift_cargo_to_orbit_target(server, session, gm, params):
    ...
    if not isinstance(target_data, dict):
        return {"success": False, "message": "Invalid target_data."}
    if not str(item_name or "").strip():
        return {"success": False, "message": "Invalid item_name."}
    qty = _as_int(params.get("qty", 1), default=None)
    if qty is None or qty <= 0:
        return {"success": False, "message": "Invalid qty."}
