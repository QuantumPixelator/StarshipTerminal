...
def load_base_prices():
    ...
    try:
        catalog_text = read_default_catalog_text("items.txt")
        if not isinstance(catalog_text, str):
            raise ValueError("Missing SQLite catalog text: items.txt")
        ...
        logger.exception("Error loading items.txt")

    if not base_prices:
        raise RuntimeError("No active items found in SQLite catalog: items.txt")
...
class Planet:
    ...
    def fluctuate_prices(self):
        ...
        # Standard items: controlled drift to preserve identity while still rotating deals.
        for item in self.item_modifiers:
            variance = random.uniform(0.90, 1.12)
            self.item_modifiers[item] = max(
                60, min(185, int(self.item_modifiers[item] * variance))
            )

        # Smuggling items
        for item, data in self.smuggling_inventory.items():
            variance = random.uniform(0.72, 1.32)
            ...
            data["modifier"] = max(90, min(520, int(data["modifier"] * variance)))
    ...
    def get_info(self):
        ...
        return {
            "planet_id": int(getattr(self, "planet_id", 0)),
            "name": self.name,
...
def _rebalance_planet_economy(planets):
    ...
    # Final catalog pass:
    # - Urth remains the only full-catalog planet.
    # - Other planets keep a distinct rotating subset with at least one bargain and one premium listing.
    all_items = [k for k in base_prices.keys() if k]
    urth = next((p for p in planets if p.name == "Urth"), None)
    if urth:
        for item in all_items:
            urth.item_modifiers[item] = 100

    all_planet_names = {p.name for p in planets}
    for planet in planets:
        if planet.name == "Urth":
            continue
        ...
        target_count = min(10, max(6, 6 + (len(planet.name) % 4)))
        seeded = random.Random(f"catalog::{planet.name}")
        ...
        # Prevent exact duplicate catalogs where possible.
        for other in planets:
            if other is planet or other.name == "Urth":
                continue
            other_items = set(getattr(other, "item_modifiers", {}).keys())
            if set(selection) == other_items and shuffled:
                swap_out = selection[-1]
                replacement = next(
                    (it for it in shuffled if it not in selection),
                    None,
                )
                if replacement:
        ...
        for item in selection:
            existing = int(planet.item_modifiers.get(item, seeded.randint(88, 132)))
            new_modifiers[item] = max(55, min(210, existing))

        if len(selection) >= 2:
            cheap_item = seeded.choice(selection)
            expensive_pool = [it for it in selection if it != cheap_item] or selection
            expensive_item = seeded.choice(expensive_pool)
            new_modifiers[cheap_item] = min(new_modifiers.get(cheap_item, 100), seeded.randint(70, 88))
            new_modifiers[expensive_item] = max(new_modifiers.get(expensive_item, 100), seeded.randint(128, 164))

        planet.item_modifiers = new_modifiers
...
def generate_planets():
    ...
    try:
        content = read_default_catalog_text("planets.txt")
        ...
        for block in blocks:
            ...
            planet_id_str = fields.get("planet id", "").strip()
            if planet_id_str.isdigit():
                planet_id = int(planet_id_str)
            else:
                # Stable fallback for legacy catalogs without explicit IDs.
                planet_id = len(planets) + 1
            ...
            if planet.is_smuggler_hub or random.random() < 0.10:
                if smuggle_item_pool:
                    ...
                    num_items = random.randint(1, 3) if planet.is_smuggler_hub else 1
                    selected = random.sample(
                        smuggle_item_pool,
                        min(num_items, len(smuggle_item_pool)),
                    )
                    for s_item in selected:
                        item_meta = dict(smuggle_item_meta.get(s_item, {}) or {})
                        base_price = int(
                            item_meta.get("base_price", base_prices.get(s_item, 500))
                        )
                        modifier, tier = _roll_smuggle_modifier(
                            s_item,
        ...
        logger.exception("Error loading planets.txt")

    if not planets:
        raise RuntimeError("No active planets found in SQLite catalog: planets.txt")
