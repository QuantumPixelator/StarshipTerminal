...
class SyncNetworkClient:
    ...
    def __init__(self, server_url: str = "ws://localhost:8765"):
        ...
        self.request_timeout_seconds = 6
    ...
    def start(self):
        ...
        # Wait for loop to be ready without burning CPU on a spin-loop.
        is_ready = self._loop_ready.wait(timeout=2)
        if not is_ready or self.loop is None:
            raise RuntimeError("Failed to initialize async network loop")
    ...
    def _run_async(self, coro):
        ...
        if not self.loop:
            raise RuntimeError("Async network loop is unavailable")
