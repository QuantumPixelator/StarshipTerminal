...
from views.travel_helpers import _calculate_travel_fuel_cost, _get_warp_travel_duration_seconds, calculate_docking_fee
...
class TravelView(arcade.View):
    ...
    def _play_sfx(self, sound_key, intensity=0.9):
        if not bool(self.network.config.get("audio_enabled")):
            return
        played = bool(play_effect_sound(str(sound_key), float(intensity)))
        if played:
            return

        fallback = {
            "planet_scan": ":resources:sounds/coin1.wav",
            "jump_charge": ":resources:sounds/jump1.wav",
            "error": ":resources:sounds/hit5.wav",
            "cancel": ":resources:sounds/upgrade5.wav",
        }.get(str(sound_key), ":resources:sounds/coin1.wav")
        try:
            sound = arcade.load_sound(fallback)
            arcade.play_sound(sound, volume=max(0.0, min(1.0, float(intensity))))
        except Exception:
            return
    ...
    def on_draw(self):
        ...
        for i, p in enumerate(self.network.planets):
            ...
            if is_sel and not is_here:
                ...
                docking_fee = calculate_docking_fee(self.network, p)
            ...
            evt = self.network.get_planet_event_cached(p.name)
            if evt:
                arcade.draw_circle_outline(
        ...
        selected_event = self.network.get_planet_event_cached(
    ...
    def on_key_press(self, key, modifiers):
        if key == arcade.key.UP:
            ...
            self._play_sfx("planet_scan", 0.8)
            ...
            self._play_sfx("jump_charge", 1.0)
    ...
    def on_mouse_motion(self, x, y, dx, dy):
        ...
        previous = int(self.selected_planet)
        closest_idx = 0
        min_dist = 999999
        for i, p in enumerate(self.network.planets):
        ...
        if min_dist < 65:
            ...
            if self.selected_planet != previous:
                self._play_sfx("planet_scan", 0.75)
