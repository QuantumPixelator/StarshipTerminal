...
def calculate_docking_fee(network, planet):
    """Compute estimated docking fee locally (mirrors server formula in ship_ops.py).

    Avoids a network call from TravelView.on_draw() so the map renders at full frame rate.
    """
    try:
        base_fee = float(network.config.get("base_docking_fee", 50))
        level_multiplier = float(network.config.get("docking_fee_ship_level_multiplier", 1.0))
        ship = network.player.spaceship
        ship_level = 1
        for i, s in enumerate(list(network.spaceships or [])):
            if getattr(s, "model", "") == getattr(ship, "model", ""):
                ship_level = i + 1
                break
        rep = int(
            getattr(network.player, "authority_standing",
                    getattr(network.player, "sector_reputation", 0))
        )
        rep_step = float(network.config.get("reputation_docking_fee_step", 0.05))
        rep_tiers = int(rep // 20)
        rep_modifier = max(0.70, min(1.40, 1.0 - (rep_tiers * rep_step)))
        event_modifier = 1.0
        if hasattr(network, "get_planet_event_cached"):
            evt = network.get_planet_event_cached(getattr(planet, "name", ""))
            if evt:
                event_modifier = max(0.70, min(1.60, float(evt.get("docking_mult", 1.0))))
        fee = int(round(base_fee * ship_level * level_multiplier * rep_modifier * event_modifier))
        return max(0, fee)
    except Exception:
        return 0
