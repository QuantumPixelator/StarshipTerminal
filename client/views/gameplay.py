...
RESOURCE_UI_ORDER = ("fuel", "ore", "tech", "bio", "rare")
RESOURCE_UI_COLORS = {
    "fuel": (250, 190, 60),
    "ore": (165, 168, 176),
    "tech": (110, 220, 255),
...
class PlanetView(arcade.View):
    def __init__(
        ...
        def _safe_config_float(key, default):
            raw = self.network.config.get(key, default)
            if raw is None:
        ...
        self.sfx_channel_volume = {
            "ui": max(
                0.0, min(1.0, _safe_config_float("audio_ui_volume", 0.7))
            ),
            "combat": max(
                0.0,
                min(1.0, _safe_config_float("audio_combat_volume", 0.7)),
            ),
            "ambient": max(
                0.0,
                min(1.0, _safe_config_float("audio_ambient_volume", 0.7)),
            ),
        }
        self.sfx_assets = {}
        self.mode = "INFO"
        ...
        self.is_docked = bool(getattr(self.network.player, "is_docked", False))
        self.menu_options = []
        ...
        self._default_combat_player_texture = self._load_combat_texture(
            ":resources:images/space_shooter/playerShip1_blue.png"
        )
        self._default_combat_target_texture = self._load_combat_texture(
...
>>>>>>> 1511b0b46872728130faad7a264914cb11dc1818
        ...
        self.resource_snapshot = {"resources": {}, "cargo": {}, "events": []}
        self.resource_snapshot_timer = 0.0
        self.resource_snapshot_interval = 1.25
        ...
        self.trade_confirm_payload = None
        self.neon_banner_seconds = max(
            1.0,
            float(self.network.config.get("commander_presence_banner_seconds", 5)),
        )
        self.presence_alert_cursor = 0.0
        self.presence_poll_elapsed = 0.0
        self._presence_poll_initialized = False
        self.economy_alert_cursor = 0.0
        self.economy_alert_poll_elapsed = 0.0
        self.maintenance_tick_elapsed = 0.0
        self.auto_refuel_interval = 1.0
        self.interest_tick_interval = 15.0
        self.regen_tick_interval = 20.0
        self.stipend_tick_interval = 20.0
        self.resource_production_tick_interval = 15.0
        self.resource_interest_tick_interval = 15.0
        self.crew_pay_tick_interval = 30.0
        self._maintenance_next = {
            "auto_refuel": 0.0,
            "interest": 0.0,
            "regen": 0.0,
            "stipend": 0.0,
        ...
        self.resource_trade_recipient_index = 0
        self.resource_trade_offer_resource_index = 0
        self.resource_trade_request_resource_index = 1
        self.resource_trade_offer_amount = 10
        self.resource_trade_request_amount = 10
        self.resource_trade_inbox_offers = []
        self.resource_trade_inbox_index = 0
        ...
        self.arrival_msg = f"ARRIVED IN ORBIT AROUND {planet.name.upper()}."
        ...
        self._refresh_menu_options(force_reset=True)
        self._refresh_resource_snapshot(force=True)
    ...
    def _play_sfx(self, channel, sound_key):
        ...
        if not sound:
            # Route through the integrated sound catalog when a local fallback key
            # is not preloaded in PlanetView's lightweight cache.
            alias_map = {
                "ui_move": "menu_select",
                "ui_confirm": "confirm",
                "ambient_dock": "planet_docking",
            }
            mapped_key = alias_map.get(key, key)
            volume = float(self.sfx_channel_volume.get(str(channel), 0.7))
            if volume <= 0.0:
                return
            played = bool(play_effect_sound(mapped_key, max(0.0, min(1.5, volume))))
            if played:
                return

            # Never go silent if catalog playback fails; use local built-in cues.
            fallback_by_channel = {
                "ui": "ui_confirm",
                "combat": "combat_hit",
                "ship": "ui_confirm",
                "ambient": "ambient_dock",
                "planet": "ambient_dock",
            }
            fallback_key = fallback_by_channel.get(str(channel), "ui_confirm")
            fallback_sound = self.sfx_assets.get(fallback_key)
            if not fallback_sound:
                return
            try:
                arcade.play_sound(fallback_sound, volume=volume)
            except Exception:
                return
            return
        volume = float(self.sfx_channel_volume.get(str(channel), 0.7))
    ...
    def _menu_option_sfx(self, option_name):
        """Return a contextual sound key per top-level menu area."""
        opt = str(option_name or "").upper()
        if opt == "MARKET":
            return "purchase"
        if opt == "RESOURCES":
            return "notification"
    ...
    def _services_locked(self):
        is_barred, _ = self.network.check_barred(self.network.current_planet.name)
        return bool(is_barred or (not self.is_docked))

    def _resource_key(self):
        return RESOURCE_UI_ORDER[max(0, min(int(self.resource_selected_index), len(RESOURCE_UI_ORDER) - 1))]

    def _resource_amount(self, key):
        resources = dict((self.resource_snapshot or {}).get("resources", {}) or {})
        try:
            return int(resources.get(str(key), 0) or 0)
        except Exception:
            return 0

    def _resource_capacity(self, key):
        cargo = dict((self.resource_snapshot or {}).get("cargo", {}) or {})
        row = dict(cargo.get(str(key), {}) or {})
        max_capacity = row.get("max_capacity", 0)
        try:
            return int(max_capacity or 0)
        except Exception:
            return 0

    def _resource_market_events_line(self):
        events = list((self.resource_snapshot or {}).get("events", []) or [])
        if not events:
            return "NO ACTIVE ECONOMY EVENTS"
        pieces = []
        for evt in events[:2]:
            evt_type = str(evt.get("event_type", "EVENT") or "EVENT").replace("_", " ")
            resource = str(evt.get("affected_resource", "ALL") or "ALL").upper()
            mult = float(evt.get("multiplier", 1.0) or 1.0)
            pieces.append(f"{evt_type}: {resource} x{mult:.2f}")
        return " | ".join(pieces)

    def _refresh_resource_snapshot(self, force=False):
        if not force and self.resource_snapshot_timer < float(self.resource_snapshot_interval):
            return
        try:
            snapshot = self.network.get_resource_snapshot() or {}
    ...
    def _open_resource_slider_prompt(self, action, resource_key, max_qty):
        max_qty = max(0, int(max_qty))
        if max_qty <= 0:
            self.market_message = "NO AVAILABLE QUANTITY FOR THIS RESOURCE ACTION."
            return
        self.action_slider_context = "RESOURCE_MARKET"
        self.action_slider_kind = str(action or "").upper()
        self.action_slider_item_name = str(resource_key or "").lower()
        self.action_slider_direction = None
    ...
    def _open_resource_trade_offer_prompt(self):
        recipients = list(self.network.get_other_players() or [])
        if not recipients:
            self.market_message = "NO OTHER COMMANDERS AVAILABLE FOR TRADE."
            return
        self.resource_trade_recipient_index = max(
            0,
            min(int(self.resource_trade_recipient_index), len(recipients) - 1),
        )
        selected_resource = self._resource_key()
        self.resource_trade_offer_resource_index = RESOURCE_UI_ORDER.index(selected_resource)
        self.resource_trade_request_resource_index = (
            self.resource_trade_offer_resource_index + 1
        ) % len(RESOURCE_UI_ORDER)
        self.resource_trade_offer_amount = max(1, int(self._resource_amount(selected_resource) // 4) or 1)
        self.resource_trade_request_amount = max(1, int(self.resource_trade_offer_amount))
        self.prompt_mode = "RESOURCE_TRADE_OFFER"

    def _open_resource_trade_inbox_prompt(self):
        offers = list(self.network.get_resource_trade_offers() or [])
        self.resource_trade_inbox_offers = offers
        self.resource_trade_inbox_index = 0
        if not offers:
            self.market_message = "NO PENDING RESOURCE TRADE OFFERS."
            return
        self.prompt_mode = "RESOURCE_TRADE_INBOX"

    def _draw_resource_hud_strip(self):
        resources = dict((self.resource_snapshot or {}).get("resources", {}) or {})
        if not resources:
            return
        ...
        for idx, key in enumerate(RESOURCE_UI_ORDER):
            amount = int(resources.get(key, 0) or 0)
            cap = self._resource_capacity(key)
            color = RESOURCE_UI_COLORS.get(key, COLOR_SECONDARY)
            x = strip_x + (idx * (card_w + gap))
            is_selected = idx == int(self.resource_selected_index)
            border = COLOR_PRIMARY if is_selected else COLOR_TEXT_DIM
            arcade.draw_lbwh_rectangle_filled(x, strip_y, card_w, card_h, (8, 12, 18, 225))
            arcade.draw_lbwh_rectangle_outline(x, strip_y, card_w, card_h, border, 1)
            arcade.draw_lbwh_rectangle_filled(x + 2, strip_y + card_h - 4, card_w - 4, 2, color)
    ...
    def _refresh_menu_options(self, force_reset=False):
        old_option = None
        if self.menu_options and 0 <= int(self.selected_menu) < len(self.menu_options):
            old_option = self.menu_options[self.selected_menu]

        options = []
        if self.is_docked:
            options.append("UNDOCK")
            options.append("MARKET")
            options.append("RESOURCES")
            if self.network.current_planet.bank:
                options.append("BANK")
            options.append("REFUEL")
            options.append("SYSTEMS")
        ...
        if force_reset or (old_option not in self.menu_options):
            self.selected_menu = 0
        else:
            self.selected_menu = self.menu_options.index(old_option)

        if self._services_locked() and self.mode in {
            "MARKET",
            "RESOURCES",
            "BANK",
            "REFUEL",
    ...
    def _load_combat_texture(self, texture_path):
        if not texture_path:
            return None
        try:
    ...
    def _ship_model_to_texture_path(self, model_name):
        raw_model = str(model_name or "").strip().lower()
        if not raw_model:
            return ""
        slug = re.sub(r"[^a-z0-9]+", "_", raw_model).strip("_")
        if not slug:
            return ""
        return os.path.join("assets", "ships", f"{slug}.png")

    def _get_combat_ship_texture(self, model_name, fallback_texture):
        model_key = str(model_name or "").strip().lower()
        if not model_key:
            return fallback_texture
        if model_key in self._combat_ship_texture_cache:
            return self._combat_ship_texture_cache[model_key] or fallback_texture

        ship_texture = self._load_combat_texture(
            self._ship_model_to_texture_path(model_name)
        )
        self._combat_ship_texture_cache[model_key] = ship_texture
        return ship_texture or fallback_texture

    def _extract_target_ship_model(self, target_data):
        target = target_data if isinstance(target_data, dict) else {}
        target_type = str(target.get("type", "")).upper()

        if target_type == "NPC":
            target_obj = target.get("obj")
            if target_obj is not None and hasattr(target_obj, "get_info"):
                info = target_obj.get_info() or {}
                ship_info = info.get("spaceship", {})
                return ship_info.get("model")
            target_stats = target.get("stats", {})
            return target_stats.get("ship")
    ...
    def _get_visible_market_items(self):
        ...
        # Fetch all market prices in one round-trip (cached until planet change)
        all_prices = self.network.get_planet_market_prices(planet.name) or {}

        for item_name, base_price in dict(getattr(planet, "items", {}) or {}).items():
            price_data = all_prices.get(item_name)
            if price_data:
                effective_price = int(price_data.get("buy", base_price) or base_price)
            else:
        ...
        for s_item, data in smuggling_inventory.items():
            ...
            if smuggle_base <= 0:
                continue
            price_data = all_prices.get(s_item)
            if price_data:
                effective_smuggle = int(price_data.get("buy", smuggle_base) or smuggle_base)
            else:
    ...
    def _confirm_bank_input(self):
        ...
        if success:
            self._play_sfx("ui", "credits_transfer")
    ...
    def _draw_help_overlay(self):
        ...
        mode_lines = {
            ...
            "RESOURCES": [
                "RESOURCES: [1-5] SELECT FUEL/ORE/TECH/BIO/RARE",
                "RESOURCES: [B] BUY SELECTED | [V] SELL SELECTED",
                "RESOURCES: [T] SEND PLAYER TRADE OFFER | [Y] OPEN OFFER INBOX",
                "RESOURCES: [R] REFRESH SNAPSHOT",
            ],
            "ORBIT": [
    ...
    def _load_named_ship_texture(self, ship_key):
        safe = str(ship_key or "").strip().lower()
        if safe in self._combat_ship_texture_cache:
            return self._combat_ship_texture_cache[safe]
        path = os.path.join("assets", "ships", f"{safe}.png")
        try:
            tex = arcade.load_texture(path)
        except Exception:
            tex = None
        self._combat_ship_texture_cache[safe] = tex
        return tex

    def _resolve_ship_visual_key(self, ship_name, is_target=False):
        lowered = str(ship_name or "").strip().lower()
        family = "fighter"
        for cls in ("fighter", "bomber", "cruiser", "freighter", "flagship"):
            if cls in lowered:
                family = cls
                break
        side = "npc" if is_target else "player"
    ...
    def _draw_combat_window(self):
        ...
        self.combat_player_texture = self._load_named_ship_texture(
            self._resolve_ship_visual_key(getattr(p_ship, "model", "fighter"), False)
        )
        target_hint = str(s.get("target_name") or s.get("target_type") or "fighter")
        self.combat_target_texture = self._load_named_ship_texture(
            self._resolve_ship_visual_key(target_hint, True)
        )
    ...
    def on_update(self, delta_time):
        ...
        self.presence_poll_elapsed += delta_time
        self.economy_alert_poll_elapsed += delta_time
        self.maintenance_tick_elapsed += delta_time
        self.resource_snapshot_timer += delta_time

        dock_state = bool(getattr(self.network.player, "is_docked", self.is_docked))
        if dock_state != self.is_docked:
            self.is_docked = dock_state
            self._refresh_menu_options()

        if self._neon_banner_active:
            self._neon_banner_active["ttl"] = max(
                0.0, float(self._neon_banner_active.get("ttl", 0.0)) - float(delta_time)
            )
            if self._neon_banner_active["ttl"] <= 0:
                self._neon_banner_active = None
        self._activate_next_presence_banner()

        if self.presence_poll_elapsed >= 1.0:
            self.presence_poll_elapsed = 0.0
            try:
                alerts = self.network.get_presence_alerts(self.presence_alert_cursor)
            except Exception:
                alerts = []
            if not self._presence_poll_initialized:
                self._presence_poll_initialized = True
                for alert in list(alerts or []):
                    ts = float(alert.get("timestamp", 0.0) or 0.0)
                    if ts > self.presence_alert_cursor:
                        self.presence_alert_cursor = ts
                alerts = []
            current_name = str(getattr(self.network.player, "name", "") or "").strip().lower()
            for alert in list(alerts or []):
                ts = float(alert.get("timestamp", 0.0) or 0.0)
                if ts > self.presence_alert_cursor:
                    self.presence_alert_cursor = ts
                commander = str(alert.get("commander", "") or "").strip()
                if not commander or commander.lower() == current_name:
                    continue
                event_name = str(alert.get("event", "")).lower()
                if event_name == "login":
                    msg = f"SECTOR RELAY: COMMANDER {commander.upper()} LOGGED IN"
                    color = (48, 255, 196)
                elif event_name == "logout":
                    msg = f"SECTOR RELAY: COMMANDER {commander.upper()} LOGGED OUT"
                    color = (255, 118, 74)
                else:
                    continue
                duration = float(alert.get("display_seconds", self.neon_banner_seconds) or self.neon_banner_seconds)
                self._queue_presence_banner(msg, color, duration=duration)

        if self.economy_alert_poll_elapsed >= 1.5:
            self.economy_alert_poll_elapsed = 0.0
            try:
                eco_alerts = self.network.get_economy_alerts(self.economy_alert_cursor)
            except Exception:
                eco_alerts = []
            current_name = str(getattr(self.network.player, "name", "") or "").strip().lower()
            for alert in list(eco_alerts or []):
                ts = float(alert.get("timestamp", 0.0) or 0.0)
                if ts > self.economy_alert_cursor:
                    self.economy_alert_cursor = ts
                commander = str(alert.get("commander", "") or "").strip()
                message = str(alert.get("message", "") or "").strip()
                if not message:
                    continue
                event_key = str(alert.get("event", "") or "").lower()
                color = (255, 214, 96)
                if "rebellion" in message.lower():
                    color = COLOR_ACCENT
                elif "interest" in event_key:
                    color = (120, 230, 140)
                elif "trade" in event_key:
                    color = (110, 220, 255)

                if commander and commander.lower() == current_name:
                    display = f"ECON UPDATE: {message.upper()}"
                elif commander:
                    display = f"{commander.upper()} :: {message.upper()}"
                else:
                    display = f"SECTOR ECON: {message.upper()}"
                duration = float(alert.get("display_seconds", self.neon_banner_seconds) or self.neon_banner_seconds)
                self._queue_presence_banner(self._clamp_text(display, 120), color, duration=duration)

        # Housekeeping tasks are throttled and staggered to avoid per-frame request bursts.
        mt = self.maintenance_tick_elapsed

        if mt >= self._maintenance_next["auto_refuel"]:
            self.network.check_auto_refuel()
            self._maintenance_next["auto_refuel"] = mt + self.auto_refuel_interval

        if mt >= self._maintenance_next["interest"]:
            success, msg = self.network.payout_interest()
            if success:
                self.arrival_msg = msg
                self.arrival_msg_timer = 5.0
            self._maintenance_next["interest"] = mt + self.interest_tick_interval

        if mt >= self._maintenance_next["regen"]:
            ...
            self._maintenance_next["regen"] = mt + self.regen_tick_interval

        if mt >= self._maintenance_next["stipend"]:
            stipend_success, stipend_msg = self.network.process_commander_stipend()
            if stipend_success:
                self.arrival_msg = stipend_msg
                self.arrival_msg_timer = 4.0
            self._maintenance_next["stipend"] = mt + self.stipend_tick_interval

        if mt >= self._maintenance_next["resource_production"]:
            production_success, production_msg = self.network.process_resource_production()
            if production_success and production_msg:
                self.arrival_msg = str(production_msg)
                self.arrival_msg_timer = 4.0
            self._maintenance_next["resource_production"] = mt + self.resource_production_tick_interval

        if mt >= self._maintenance_next["resource_interest"]:
            resource_interest_success, resource_interest_msg = self.network.payout_resource_interest()
            if resource_interest_success and resource_interest_msg:
                self.arrival_msg = str(resource_interest_msg)
                self.arrival_msg_timer = 4.0
            self._maintenance_next["resource_interest"] = mt + self.resource_interest_tick_interval

        if mt >= self._maintenance_next["crew_pay"]:
            ...
            if crew_pay_success and crew_pay_msg:
                ...
                self.arrival_msg_timer = 5.0
            self._maintenance_next["crew_pay"] = mt + self.crew_pay_tick_interval
    ...
    def _draw_perf_hud(self):
        ...
        if self.mode == "INFO":
            ...
            self.header_txt.text = "STRATEGIC RESOURCE EXCHANGE"
            self.header_txt.draw()

            selected_resource = self._resource_key()
            selected_amount = self._resource_amount(selected_resource)
            selected_capacity = self._resource_capacity(selected_resource)

            panel_x = content_x
            panel_y = content_y - 470
            panel_w = SCREEN_WIDTH - panel_x - 42
            panel_h = 420
            arcade.draw_lbwh_rectangle_filled(panel_x, panel_y, panel_w, panel_h, (8, 14, 22, 218))
            arcade.draw_lbwh_rectangle_outline(panel_x, panel_y, panel_w, panel_h, COLOR_SECONDARY, 2)
            ...
            line_y = panel_y + panel_h - 68
            for idx, key in enumerate(RESOURCE_UI_ORDER):
                color = RESOURCE_UI_COLORS.get(key, COLOR_TEXT_DIM)
                amount = self._resource_amount(key)
                capacity = self._resource_capacity(key)
                marker = ">" if idx == int(self.resource_selected_index) else " "
                amount_text = f"{amount:,}"
                if capacity > 0:
                    amount_text = f"{amount:,}/{capacity:,}"
                arcade.Text(
                    f"{marker} [{idx + 1}] {key.upper():<5}  {amount_text}",
                    panel_x + 18,
                    line_y - (idx * 26),
                    color,
                    13,
                    font_name=self.font_ui_bold if idx == int(self.resource_selected_index) else self.font_ui,
                ).draw()

            event_text = self._resource_market_events_line()
            arcade.Text(
                f"MARKET EVENTS: {event_text}",
                panel_x + 18,
                panel_y + 132,
                COLOR_TEXT_DIM,
                11,
            ...
            selected_color = RESOURCE_UI_COLORS.get(selected_resource, COLOR_SECONDARY)
            selected_detail = f"SELECTED: {selected_resource.upper()}  HOLDING: {selected_amount:,}"
            if selected_capacity > 0:
                selected_detail += f" / {selected_capacity:,}"
            arcade.Text(
                selected_detail,
                panel_x + 18,
                panel_y + 86,
            ...
            arcade.Text(
                "[1-5] SELECT RESOURCE  [B] BUY  [V] SELL  [T] TRADE OFFER  [R] REFRESH  [ESC] BACK",
                panel_x + 18,
                panel_y + 44,
                COLOR_SECONDARY,
                12,
            ...
            if self.market_message:
                arcade.Text(
                    self._clamp_text(self.market_message, 128),
                    panel_x + 18,
                    panel_y + 16,
                    self._message_style(self.market_message_type)["color"],
                    12,
                    font_name=self.font_ui_bold,
                ).draw()
            ...
            planet_event = self.network.get_planet_event_cached(planet.name)
            if planet_event:
                arcade.Text(
            ...
            if not self.orbital_targets:
                ...
                for i, target in enumerate(self.orbital_targets):
                    ...
                    if is_sel:
                        # Detail Panel
                        panel_x = content_x + 500
                        panel_y = content_y - 360
                        arcade.draw_lbwh_rectangle_filled(
                            panel_x, panel_y, 560, 390, (10, 15, 20, 240)
                        )
                        arcade.draw_lbwh_rectangle_outline(
                            panel_x, panel_y, 560, 390, COLOR_PRIMARY, 2
                        )
                        ...
                        # For PLAYER targets use the compact stats dict; the
                        # old raw_data blob is no longer sent by the server.
                        player_stats = (
                            target.get("stats", {}) if target["type"] == "PLAYER" else {}
                        )
                        ship_info = (info or {}).get("spaceship", {}) if info else {}
                        ...
                        if target["type"] == "NPC":
                            ...
                            # Other players from compact stats dict
                            s_model = player_stats.get("ship", "Unknown")
                            s_integ = int(player_stats.get("integrity", 100))
                            s_shields = f"{player_stats.get('shields', 0)} / {player_stats.get('max_shields', player_stats.get('shields', 0))}"
                            s_defenders = f"{player_stats.get('defenders', 0)} / {player_stats.get('max_defenders', player_stats.get('defenders', 0))}"
                        ...
                        if target.get("is_abandoned"):
                            ...
                            if cargo_items:
                                ...
                                for display_idx, item_idx in enumerate(
                                    ...
                                    row_y = panel_y + 166 - display_idx * 20
        ...
        if self.prompt_mode == "ACTION_SLIDER":
            ...
            context_label = (
                ...
                if self.action_slider_context in {"MARKET", "RESOURCE_MARKET"}
        ...
        if self.prompt_mode == "TRADE_CONFIRM":
            payload = dict(self.trade_confirm_payload or {})
            action = str(payload.get("action", "")).upper()
            item_name = str(payload.get("item_name", "CARGO"))
            qty = max(1, int(payload.get("qty", 1)))
            unit_price = max(0, int(payload.get("unit_price", 0)))
            total_price = max(0, int(payload.get("total_price", unit_price * qty)))
            verb = "purchase" if action == "BUY" else "sell"
            ...
            box_w, box_h = 640, 300
            bx, by = SCREEN_WIDTH // 2 - box_w // 2, SCREEN_HEIGHT // 2 - box_h // 2
            arcade.draw_lbwh_rectangle_filled(bx, by, box_w, box_h, (8, 14, 24, 248))
            arcade.draw_lbwh_rectangle_outline(bx, by, box_w, box_h, COLOR_PRIMARY, 2)
            ...
            convo = (
                f"Commander, {verb} {qty} x {item_name} at "
                f"{self._format_usd_amount(unit_price)} each?\n"
                f"Total {verb} value: {self._format_usd_amount(total_price)}"
            )
            arcade.Text(
                convo,
                bx + box_w // 2,
        ...
        if self.prompt_mode == "RESOURCE_TRADE_OFFER":
            recipients = list(self.network.get_other_players() or [])
            if not recipients:
                self.prompt_mode = None
            else:
                idx = int(self.resource_trade_recipient_index) % len(recipients)
                recipient = str(recipients[idx])
                offer_resource = RESOURCE_UI_ORDER[
                    int(self.resource_trade_offer_resource_index) % len(RESOURCE_UI_ORDER)
                ]
                request_resource = RESOURCE_UI_ORDER[
                    int(self.resource_trade_request_resource_index) % len(RESOURCE_UI_ORDER)
                ]
                offer_amount = max(1, int(self.resource_trade_offer_amount))
                request_amount = max(1, int(self.resource_trade_request_amount))
                offer_available = int(self._resource_amount(offer_resource))
                ...
                by = SCREEN_HEIGHT // 2 - box_h // 2
                arcade.draw_lbwh_rectangle_filled(
                    bx, by, box_w, box_h, (8, 14, 24, 248)
                )
                arcade.draw_lbwh_rectangle_outline(
                    bx, by, box_w, box_h, COLOR_PRIMARY, 2
                )

                arcade.Text(
                    "PLAYER RESOURCE TRADE OFFER",
                    bx + box_w // 2,
                    by + box_h - 46,
                    COLOR_PRIMARY,
                    20,
                ...
                arcade.Text(
                    f"RECIPIENT: {recipient.upper()}  ([LEFT]/[RIGHT])",
                    bx + 28,
                    by + box_h - 92,
                    COLOR_SECONDARY,
                    14,
                ...
                arcade.Text(
                    f"OFFER: {offer_amount:,} {offer_resource.upper()}  (AVAILABLE {offer_available:,})  [A/D RESOURCE] [UP/DOWN AMOUNT]",
                    bx + 28,
                    by + box_h - 144,
                    RESOURCE_UI_COLORS.get(offer_resource, COLOR_TEXT_DIM),
                    13,
                    font_name=self.font_ui,
                ).draw()

                arcade.Text(
                    f"REQUEST: {request_amount:,} {request_resource.upper()}  [J/L RESOURCE] [PGUP/PGDN AMOUNT]",
                    bx + 28,
                    by + box_h - 182,
                    RESOURCE_UI_COLORS.get(request_resource, COLOR_TEXT_DIM),
                    13,
                    font_name=self.font_ui,
                ).draw()

                arcade.Text(
                    "[ENTER] SEND OFFER   [ESC] CANCEL",
                    bx + box_w // 2,
                    by + 24,
                    COLOR_TEXT_DIM,
                    11,
        ...
        if self.prompt_mode == "RESOURCE_TRADE_INBOX":
            offers = list(self.resource_trade_inbox_offers or [])
            if not offers:
                self.prompt_mode = None
            else:
                idx = max(0, min(int(self.resource_trade_inbox_index), len(offers) - 1))
                selected = dict(offers[idx] or {})
                from_player = str(selected.get("from_player") or "UNKNOWN")
                offer = dict(selected.get("offer") or {})
                request = dict(selected.get("request") or {})
                offer_res = str(offer.get("resource") or "?").upper()
                offer_amt = int(offer.get("amount") or 0)
                request_res = str(request.get("resource") or "?").upper()
                request_amt = int(request.get("amount") or 0)
                ...
                arcade.draw_lbwh_rectangle_outline(
                    bx, by, box_w, box_h, COLOR_SECONDARY, 2
                )

                arcade.Text(
                    f"RESOURCE TRADE INBOX ({idx + 1}/{len(offers)})",
                    bx + box_w // 2,
                    by + box_h - 44,
                    COLOR_PRIMARY,
                    20,
                ...
                arcade.Text(
                    f"THEY OFFER: {offer_amt:,} {offer_res}",
                    bx + 28,
                    by + box_h - 142,
                    RESOURCE_UI_COLORS.get(str(offer.get("resource") or "").lower(), COLOR_TEXT),
                    14,
                    font_name=self.font_ui,
                ).draw()
                arcade.Text(
                    f"THEY REQUEST: {request_amt:,} {request_res}",
                    bx + 28,
                    by + box_h - 176,
                    RESOURCE_UI_COLORS.get(str(request.get("resource") or "").lower(), COLOR_TEXT_DIM),
                    14,
                    font_name=self.font_ui,
                ).draw()
                arcade.Text(
                    "[UP/DOWN] SELECT | [A OR ENTER] ACCEPT | [D] REJECT | [R] REFRESH | [ESC] CLOSE",
                    bx + box_w // 2,
                    by + 16,
                    COLOR_TEXT_DIM,
                    11,
    ...
    def _execute_menu_selection(self):
        ...
        self._play_sfx("ui", self._menu_option_sfx(opt))

        if opt == "DOCK":
            success, msg = self.network.dock_current_planet()
            self.arrival_msg = str(msg).upper()
            ...
            success, msg = self.network.undock_current_planet()
            self.arrival_msg = str(msg).upper()
            self.arrival_msg_timer = 3.0
            if success:
        ...
        if opt == "LOGOUT":
            ...
            if self.is_docked:
                self.network.undock_current_planet()
                self.is_docked = False
                self._refresh_menu_options()
            ...
            if self.is_docked:
                self.arrival_msg = "UNDOCK FIRST TO SCAN OR ENGAGE COMBAT."
                self.arrival_msg_timer = 2.4
                return
            self.mode = "ORBIT"
            self._refresh_planet_finance_cache(force=True)
            ...
            if self._services_locked():
                self.arrival_msg = "BANK ACCESS REQUIRES DOCKED CLEARANCE."
            ...
            if self._services_locked():
                self.arrival_msg = "REFUELING REQUIRES DOCKED CLEARANCE."
                self.arrival_msg_timer = 2.8
                return
            self.mode = "REFUEL"
        elif opt == "MARKET":
            ...
            if self._services_locked():
                self.arrival_msg = "RESOURCE EXCHANGE REQUIRES DOCKED CLEARANCE."
                self.arrival_msg_timer = 2.8
                return
            self.mode = "RESOURCES"
            self.market_message = ""
            ...
            if self._services_locked():
                self.arrival_msg = "SHIP SERVICES REQUIRE DOCKED CLEARANCE."
                self.arrival_msg_timer = 2.8
                return
            self.mode = "SYSTEMS"
            self.system_message = ""
            ...
            if self._services_locked():
                self.arrival_msg = "SHIPYARD ACCESS REQUIRES DOCKED CLEARANCE."
                self.arrival_msg_timer = 2.8
                return
            self.mode = "SHIPYARD"
            self.selected_ship_index = 0
            ...
            if self._services_locked():
                self.arrival_msg = "COMMUNICATION HUB REQUIRES DOCKED CLEARANCE."
                self.arrival_msg_timer = 2.8
                return
            self.mode = "MAIL"
    ...
    def _get_owned_planets_current_commander(self):
        ...
        if not owned:
            ...
            for planet_key in owned_dict.keys():
                planet_name = None
                for planet in list(getattr(self.network, "planets", []) or []):
                    if str(getattr(planet, "planet_id", "")) == str(planet_key):
                        planet_name = str(getattr(planet, "name", ""))
                        break
                owned.append(str(planet_name or planet_key))
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.F2 and not self.combat_session:
            self.audio_enabled = not bool(self.audio_enabled)
            self.network.config["audio_enabled"] = bool(self.audio_enabled)
            if hasattr(self.network, "save_local_audio_preferences"):
                try:
                    self.network.save_local_audio_preferences()
                except Exception:
                    pass
            self.arrival_msg = (
                "AUDIO CHANNELS ONLINE." if self.audio_enabled else "AUDIO MUTED."
            )
            self.arrival_msg_timer = 2.0
            return
        ...
        if self.combat_session:
            ...
            if key in (arcade.key.LEFT, arcade.key.A):
                ...
                self._play_sfx("combat", "target_acquired")
                return
        ...
        if self.prompt_mode == "TRADE_CONFIRM":
            if key == arcade.key.ESCAPE:
                ...
                return
            if key in (arcade.key.ENTER, arcade.key.RETURN):
                self._execute_trade_confirmation()
                return
            return
        ...
        if self.prompt_mode == "RESOURCE_TRADE_OFFER":
            ...
            if key == arcade.key.LEFT:
                self.resource_trade_recipient_index = (
                    int(self.resource_trade_recipient_index) - 1
                ) % len(recipients)
                return
            ...
            if key in (arcade.key.A,):
                self.resource_trade_offer_resource_index = (
                    int(self.resource_trade_offer_resource_index) - 1
                ) % len(RESOURCE_UI_ORDER)
                return
            if key in (arcade.key.D,):
                self.resource_trade_offer_resource_index = (
                    int(self.resource_trade_offer_resource_index) + 1
                ) % len(RESOURCE_UI_ORDER)
                return
            if key in (arcade.key.J,):
                self.resource_trade_request_resource_index = (
                    int(self.resource_trade_request_resource_index) - 1
                ) % len(RESOURCE_UI_ORDER)
                return
            if key in (arcade.key.L,):
                self.resource_trade_request_resource_index = (
                    int(self.resource_trade_request_resource_index) + 1
                ) % len(RESOURCE_UI_ORDER)
                return
            if key == arcade.key.UP:
                self.resource_trade_offer_amount = min(9999, int(self.resource_trade_offer_amount) + 1)
                return
            if key == arcade.key.DOWN:
                self.resource_trade_offer_amount = max(1, int(self.resource_trade_offer_amount) - 1)
                return
            if key == arcade.key.PAGEUP:
                self.resource_trade_request_amount = min(9999, int(self.resource_trade_request_amount) + 1)
                return
            if key == arcade.key.PAGEDOWN:
                self.resource_trade_request_amount = max(1, int(self.resource_trade_request_amount) - 1)
                return
            if key in (arcade.key.ENTER, arcade.key.RETURN):
                offer_resource = RESOURCE_UI_ORDER[int(self.resource_trade_offer_resource_index) % len(RESOURCE_UI_ORDER)]
                request_resource = RESOURCE_UI_ORDER[int(self.resource_trade_request_resource_index) % len(RESOURCE_UI_ORDER)]
                available_offer = int(self._resource_amount(offer_resource))
                offer_amount = min(available_offer, max(1, int(self.resource_trade_offer_amount)))
                request_amount = max(1, int(self.resource_trade_request_amount))
                if offer_amount <= 0:
                    self.market_message = "INSUFFICIENT RESOURCE FOR OFFER."
                    self.prompt_mode = None
                    return
                recipient = recipients[int(self.resource_trade_recipient_index) % len(recipients)]
                success, msg, _ = self.network.create_resource_trade_offer(
                    recipient,
                    {"resource": offer_resource, "amount": int(offer_amount)},
                    {"resource": request_resource, "amount": int(request_amount)},
                )
                self.market_message = str(msg or "").upper()
                self._refresh_resource_snapshot(force=True)
                if success:
                    self._play_sfx("ui", "confirm")
                    self.network.save_game()
                else:
                    self._play_sfx("ui", "error")
                self.prompt_mode = None
        ...
        if self.prompt_mode == "RESOURCE_TRADE_INBOX":
            ...
            if not offers:
                ...
                self.market_message = "NO PENDING RESOURCE TRADE OFFERS."
                return
            self.resource_trade_inbox_index = max(
                0,
                min(int(self.resource_trade_inbox_index), len(offers) - 1),
            )
            if key == arcade.key.ESCAPE:
                self.prompt_mode = None
                return
            if key == arcade.key.UP:
                self.resource_trade_inbox_index = (
                    int(self.resource_trade_inbox_index) - 1
                ) % len(offers)
                return
            if key == arcade.key.DOWN:
                self.resource_trade_inbox_index = (
                ...
                return
            if key == arcade.key.R:
                self._open_resource_trade_inbox_prompt()
                return
            if key in (arcade.key.A, arcade.key.ENTER, arcade.key.RETURN, arcade.key.D):
                selected = dict(offers[int(self.resource_trade_inbox_index)] or {})
                offer_id = int(selected.get("id", 0) or 0)
                decision = "accept" if key in (arcade.key.A, arcade.key.ENTER, arcade.key.RETURN) else "reject"
                success, msg = self.network.respond_resource_trade_offer(offer_id, decision)
                self.market_message = str(msg or "").upper()
                if success:
                    self._play_sfx("ui", "confirm")
                    self.network.save_game()
        ...
        if self.mode == "RESOURCES":
            ...
            if key in (arcade.key._5, arcade.key.NUM_5):
                ...
                return
            if key in (arcade.key.LEFT, arcade.key.A):
                self.resource_selected_index = (
                    int(self.resource_selected_index) - 1
                ) % len(RESOURCE_UI_ORDER)
                return
            if key in (arcade.key.RIGHT, arcade.key.D):
                self.resource_selected_index = (
            ...
            if key == arcade.key.R:
                self._refresh_resource_snapshot(force=True)
                self.market_message = "RESOURCE SNAPSHOT REFRESHED."
                return
            if key == arcade.key.T:
                self._open_resource_trade_offer_prompt()
                return
            if key == arcade.key.Y:
                self._open_resource_trade_inbox_prompt()
                return
            if key == arcade.key.B:
                selected = self._resource_key()
                buy_cap = 200
                self._open_resource_slider_prompt("BUY", selected, buy_cap)
                return
            if key == arcade.key.V:
                selected = self._resource_key()
                amount = self._resource_amount(selected)
                self._open_resource_slider_prompt("SELL", selected, amount)
                return
            if key == arcade.key.ESCAPE:
                self.mode = "INFO"
                self.market_message = ""
    ...
    def _format_usd_amount(self, amount):
        return f"${float(amount):,.2f}"

    def _estimate_trade_unit_price(self, action, item_name):
        action_key = str(action or "").upper()
        item_key = str(item_name or "")
        planet = self.network.current_planet
        planet_name = str(getattr(planet, "name", "") or "")

        if action_key == "BUY":
            if item_key in dict(getattr(planet, "items", {}) or {}):
                base_price = int(planet.items.get(item_key, 0) or 0)
            else:
                smuggle = dict(getattr(planet, "smuggling_inventory", {}) or {}).get(
                    item_key, {}
                )
                base_price = int((smuggle or {}).get("price", 0) or 0)
            base_price = max(1, int(base_price or 1))
            return int(self.network.get_effective_buy_price(item_key, base_price, planet_name))

        return max(1, int(self.network.get_market_sell_price(item_key, planet_name) or 1))

    def _queue_presence_banner(self, message, color, duration=None):
        ttl = max(1.0, float(duration if duration is not None else self.neon_banner_seconds))
        self._neon_banner_queue.append(
            {
                "message": str(message),
                "color": tuple(color),
                "ttl": ttl,
    ...
    def _activate_next_presence_banner(self):
        if self._neon_banner_active or not self._neon_banner_queue:
            return
        self._neon_banner_active = self._neon_banner_queue.pop(0)

    def _draw_presence_banner(self):
        banner = self._neon_banner_active
        if not banner:
            return

        ttl = float(banner.get("ttl", 0.0))
        duration = max(0.001, float(banner.get("duration", 1.0)))
        fade = max(0.0, min(1.0, ttl / duration))
        alpha = int(210 * fade)
        glow_alpha = int(120 * fade)
        color = tuple(banner.get("color", COLOR_PRIMARY))

        h = 52
        y = SCREEN_HEIGHT - h - 12
        arcade.draw_lbwh_rectangle_filled(0, y, SCREEN_WIDTH, h, (8, 12, 20, alpha))
        arcade.draw_lbwh_rectangle_filled(0, y + h - 4, SCREEN_WIDTH, 4, (*color, min(255, glow_alpha + 70)))
        arcade.draw_line(0, y, SCREEN_WIDTH, y, (*color, glow_alpha), 2)

        arcade.Text(
            str(banner.get("message", "")),
            SCREEN_WIDTH // 2,
            y + 17,
            (*color, min(255, alpha + 40)),
            16,
            anchor_x="center",
            font_name=self.font_ui_bold,
        ).draw()
    ...
    def _confirm_action_slider(self):
        ...
        if self.action_slider_context == "ORBIT":
            ...
            if success:
                self._play_sfx("ship", "module_install")
                self.network.save_game()
            else:
                self._play_sfx("ui", "error")
            ...
            unit_price = int(self._estimate_trade_unit_price(action, item_name))
            total_price = int(max(0, unit_price * int(qty)))
            self.trade_confirm_payload = {
                "action": action,
                "item_name": item_name,
                "qty": int(qty),
            ...
            self.prompt_mode = "TRADE_CONFIRM"

        elif self.action_slider_context == "RESOURCE_MARKET":
            action = str(self.action_slider_kind or "").upper()
            resource = str(self.action_slider_item_name or "").lower()
            success, msg, data = self.network.trade_planet_resource(resource, action, int(qty))
            self.market_message = str(msg or "").upper()
            if isinstance(data, dict) and data:
                self.resource_snapshot = data
            else:
                self._refresh_resource_snapshot(force=True)
            if success:
                self._play_sfx("ui", "purchase" if action == "BUY" else "sale")
                self.network.save_game()
            else:
                self._play_sfx("ui", "error")
    ...
    def _execute_trade_confirmation(self):
        payload = dict(self.trade_confirm_payload or {})
        action = str(payload.get("action", "")).upper()
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if self.combat_session:
            ...
            if (
                ...
                if elapsed >= cooldown_hours:
                    ...
                    self._play_sfx("combat", "special_weapon_ready")
                return
        ...
        if self.prompt_mode == "TRADE_CONFIRM":
            box_w, box_h = 640, 300
            bx, by = SCREEN_WIDTH // 2 - box_w // 2, SCREEN_HEIGHT // 2 - box_h // 2
            confirm_x, confirm_y, confirm_w, confirm_h = bx + 92, by + 40, 190, 44
            cancel_x, cancel_y, cancel_w, cancel_h = bx + box_w - 282, by + 40, 190, 44

            if confirm_x <= x <= confirm_x + confirm_w and confirm_y <= y <= confirm_y + confirm_h:
                self._execute_trade_confirmation()
                return
                ...
                if not target.get("is_abandoned"):
                    ...
                    if cargo_items:
                        ...
                        for display_idx, item_idx in enumerate(
                            ...
                            row_y = panel_y + 166 - display_idx * 20
                            if (
                                panel_x + 16 <= x <= panel_x + 532
                                and row_y - 3 <= y <= row_y + 15
                            ):
        ...
        if self.mode == "MARKET":
            ...
            if bx2 <= x <= bx2 + btn_w and by <= y <= by + btn_h and max_buy > 0:
                unit_price = int(self._estimate_trade_unit_price("BUY", sel_item))
                self.trade_confirm_payload = {
                    "action": "BUY",
                    "item_name": str(sel_item),
                    "qty": int(max_buy),
                    "unit_price": int(unit_price),
                    "total_price": int(max_buy * unit_price),
                }
                self.prompt_mode = "TRADE_CONFIRM"
                return
            ...
            if sx2 <= x <= sx2 + btn_w and sy <= y <= sy + btn_h and inv_qty > 0:
                unit_price = int(self._estimate_trade_unit_price("SELL", sel_item))
                self.trade_confirm_payload = {
                    "action": "SELL",
                    "item_name": str(sel_item),
                    "qty": int(inv_qty),
                    "unit_price": int(unit_price),
                    "total_price": int(inv_qty * unit_price),
                }
                self.prompt_mode = "TRADE_CONFIRM"
                return
