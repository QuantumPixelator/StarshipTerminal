...
class AudioPlaybackIntegration:
    ...
    def __init__(self, audio_manager: Optional[AudioManager] = None):
        ...
        self.missing_sounds = set()  # Avoid repeat lookup/log spam for missing assets
    ...
    def _play_via_manager(self, sound_name: str, path: str, 
        ...
        try:
            # AudioManager.play() expects category + sound name.
            sound_config = ALL_SOUNDS.get(sound_name, {})
            category = sound_config.get("category")
            if not category:
                return False

            self.audio_manager.play(category, sound_name, volume_override=volume)
            return True
        except Exception as e:
            logger.error(f"AudioManager play failed: {e}")
            return False

    def _resolve_sound_path(self, relative_path: str) -> Optional[str]:
        """Resolve configured sound path against known project roots."""
        path_obj = Path(relative_path)
        if path_obj.is_absolute() and path_obj.exists():
            return str(path_obj)

        # Normalize path style from config (forward slashes) across platforms.
        normalized = Path(relative_path.replace("/", os.sep))
        repo_root = Path(__file__).resolve().parents[2]
        candidates = [
            repo_root / normalized,
            repo_root / "client" / normalized,
            repo_root / "server" / normalized,
        ]
    ...
    def _play_via_arcade(self, sound_name: str, path: str,
        ...
        try:
            ...
            if sound_name in self.missing_sounds:
                return False
            ...
            if sound_name not in self.sound_cache:
                ...
                if not resolved_path:
                    self.missing_sounds.add(sound_name)
                    logger.warning(
                        "Sound asset missing for %s (configured path: %s)",
                        sound_name,
                        path,
                    )
                    return False
                ...
                try:
                    sound = arcade.load_sound(resolved_path)
                    self.sound_cache[sound_name] = sound
                except Exception as e:
                    logger.error(
                        "Failed to load sound %s from %s: %s",
                        sound_name,
                        resolved_path,
                        e,
                    )
            ...
            # Play the sound — avoid `looping` kwarg which isn't supported in
            # older Arcade versions; use the sound object's own play() instead.
            try:
                player = arcade.play_sound(sound, volume=volume, looping=looping)
            except TypeError:
                # Fallback: call the sound object's play method directly
